...
if (isset($_POST['submit']) && $_POST['submit'] == 'Proceed') {
    ...
    if ($output == 1) {
        ...
        if ($report_for == 'broker') {
            header("location:" . SITE_URL . "sales_broker_report_print.php?open=output_print&filter=" . $data_array);
            exit;
        } else {

            header("location:" . SITE_URL . "sales_report_print.php?open=output_print&filter=" . $data_array);
            exit;
        }
    } else if ($output == 3) {
        if ($report_for == 'broker') {
            header("location:" . SITE_URL . "sales_broker_report_excel.php?filter=" . $data_array);
            exit;
        } else {
            header("location:" . SITE_URL . "sales_report_excel.php?filter=" . $data_array);
            exit;
        }
    } else if ($output == 4) {
        if ($report_for == 'broker') {
            header("location:" . SITE_URL . "sales_broker_report_print.php?filter=" . $data_array);
            exit;
        } else {

            header("location:" . SITE_URL . "sales_report_print.php?filter=" . $data_array);
            exit;
        }
    }
}
