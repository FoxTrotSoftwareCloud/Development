...
class webcrd extends db{
	...
	public function select_finra_exam_status_data($masterId=null, $orderBy=0, $resultsOnly=0){
		$masterId = (is_null($masterId) ? $masterId : (int)$this->re_db_input($masterId));
		$orderBy = (int)$this->re_db_input($orderBy);
		$return = [];
		$con = "";
	...
	//--------------------------------------------------------------------------------
	// Registration Status - 07/30/22
	//--------------------------------------------------------------------------------
	public function registration_status_scan(){
		$filePathAndName = $_FILES["file_registration_status"]["tmp_name"];
		$fileData = $get_array_data = [];
		$return = $broker_id = $added = 0;
		$file_date = $import_date = "";
		
		if (file_exists($filePathAndName)){
			$total_scan = $this->load_registration_status_file($filePathAndName);

			// Cycle through SDN Data table records
			$fileData = $this->select_registration_status_data(0);

			foreach($fileData as $key=>$row) {
				$broker_id = $this->check_registration_status($row);
				
				if ($broker_id) { $added++; }
			}
			$file_name = $_FILES["file_registration_status"]["name"];
			$file_type = "Registration Status";
			$file_date = (empty($fileData[0]['file_date']) ? date("Y-m-d H:i:s") : $fileData[0]['file_date']); 
			$import_date = (empty($fileData[0]['import_date']) ? date("Y-m-d H:i:s") : $fileData[0]['import_date']); 
	...
	function load_registration_status_file($filePathAndName=''){
		if ($filePathAndName == '')
			return 0;
		
		if (substr(basename($filePathAndName),0,3)=="php" AND isset($_FILES['file_registration_status'])){
			$realFileName = $_FILES['file_registration_status']['name'];
		} else {
			$realFileName = basename($filePathAndName);
		}
		...
		$fileStream = fopen($filePathAndName, 'r');
		$fileFields = ['first_name', 'last_name', 'individual_crd_no', 'registrations', 'disclosures', 'exams', 'deficiencies', 'branch_locations', 'other_business', 
	                   'regulator_code', 'registration_category_code', 'registration_status_name', 'registration_status_type_code', 'current_status_date'];
		$fileDate = $this->getFileDate($realFileName);
		if ($fileDate!='') { 
			$fileDate = date("Y-m-d H:i:s", strtotime($fileDate));
		} else {
		...
		if ($fileStream){
			...
			while (($getData = fgetcsv($fileStream, 10000, ",")) !== FALSE) {
				...
				if (strcasecmp($getData[0],'first name') AND strcasecmp($getData[0],'last name')){
					...
					$q = "INSERT INTO `".WEBCRD_REGISTRATION_STATUS_DATA."`"
						." SET "
							.$setFields
							.",file_date = '$fileDate'"
							.",import_date = '".date("Y-m-d H:i:s")."'"
	...
	// Insert or Update BROKER MASTER per the Registration Status file
	function check_registration_status($fileRecord=[]){
		$return = $brokerId = 0;
		$result = "";
		// Skip the processing for invalid data record, Rep CRD #, Series Exam, Failed Test		
	...
	public function select_registration_status_data($masterId=null, $orderBy=0, $resultsOnly=0){
		...
		// Query data
		$q = "SELECT `at`.*"
				." FROM `".WEBCRD_REGISTRATION_STATUS_DATA."` `at`"
				." WHERE `at`.`is_delete` = 0"
					.$con
				." ORDER BY $orderByQuery"
		;
