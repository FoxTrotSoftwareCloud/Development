...
class home extends db
{
    ...
    public function select_daily_import_process_files_0($con = '')
    { //for all process files
        $return = array();
        $ins = new import();
        $files_array = array();
    ...
    public function select_gross_commission_month($con = '')
    {
        $return = array();

        $q = "SELECT SUM(commission_received) as gross_commission,MONTH(created_time) as month
            FROM " . PAYROLL_REVIEW_MASTER . " WHERE is_delete='0' AND created_time >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
          AND created_time <= NOW() " . $con . " GROUP BY MONTH(created_time)";
        $res = $this->re_db_query($q);

        if ($this->re_db_num_rows($res) > 0) {
            while ($row = $this->re_db_fetch_array($res)) {
                $return[$row['month']] = $row['gross_commission'];
            }
        }
        return $return;
    }

    public function select_net_commission_month($con = '')
    {
        $return = array();

        $q = "SELECT SUM(commission_paid) as net_commission,MONTH(created_time) as month
            FROM " . PAYROLL_REVIEW_MASTER . " WHERE is_delete='0' AND created_time >= DATE_SUB(NOW(), INTERVAL 12 MONTH)
          AND created_time <= NOW() " . $con . " GROUP BY MONTH(created_time)";
        $res = $this->re_db_query($q);
        if ($this->re_db_num_rows($res) > 0) {
            while ($row = $this->re_db_fetch_array($res)) {
                $return[$row['month']] = $row['net_commission'];
            }
        }
        return $return;
    }
