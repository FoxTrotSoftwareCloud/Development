...
class import extends db
{
                        ...
                        foreach ($DETAIL as $detail_key => $detail_val) {
                            ...
                            foreach ($detail_val as $seq_key => $seq_val) {
                                if ($seq_key == 001) {
                                    ...
                                    //echo $seq_val['customer_date_of_birth'];die;
                                    $q = "UPDATE " . IMPORT_DETAIL_DATA . " SET record_type2='" . $seq_val['record_type2'] . "',sequence_number2='" . $seq_val['sequence_number2'] . "',mutual_fund_dividend_mail_account='" . $seq_val['mutual_fund_dividend_mail_account'] . "',mutual_fund_stop_purchase_account='" . $seq_val['mutual_fund_stop_purchase_account'] . "',stop_mail_account='" . $seq_val['stop_mail_account'] . "',mutual_fund_fractional_check='" . $seq_val['mutual_fund_fractional_check'] . "',registration_line1='" . str_replace("'", "''", $seq_val['registration_line1']) . "',registration_line2='" . str_replace("'", "''", $seq_val['registration_line2']) . "',registration_line3='" . str_replace("'", "''", $seq_val['registration_line3']) . "',registration_line4='" . str_replace("'", "''", $seq_val['registration_line4']) . "',customer_date_of_birth='" . (empty(trim($seq_val['customer_date_of_birth'])) || $seq_val['customer_date_of_birth'] == '0000-00-00' ? '0000-00-00' : date('Y-m-d', strtotime($seq_val['customer_date_of_birth']))) . "',mutual_fund_account_price_schedule_code='" . $seq_val['mutual_fund_account_price_schedule_code'] . "',unused_002_1='" . $seq_val['unused_detail'] . "'" . $this->update_common_sql() . " WHERE id='" . $detail_inserted_id . "'";
                                    $res = $this->re_db_query($q);
                                    $data_status = true;
                                } else if ($seq_key == 003) {
                 ...
                 *******************************/
                    ...
                    foreach ($file_string_array as $key_string => $val_string) {
                        ...
                        if (isset($record_type) && $record_type == 'RHR') {
                            ...
                            if ($commission_record_type_code == '1' || $commission_record_type_code == '3' || $commission_record_type_code == '8' || $commission_record_type_code == '2' || $commission_record_type_code == '4') {
                    ...
                    foreach ($data_array as $main_key => $main_val) {
                        ...
                        $DETAIL = $main_val['DETAIL'];
                        foreach ($DETAIL as $detail_key => $detail_val) {
                            if ($detail_key == '1' || $detail_key == '3' || $detail_key == '8' || $detail_key == '2' || $detail_key == '4') {
                                foreach ($detail_val as $seq_key => $seq_val) {
                ...
                foreach ($commission_detail_array as $check_data_key => $check_data_val) {
                    ...
                    if ($check_data_val['resolve_exceptions'] != '') {
                        ...
                        foreach ($exceptionArray as $errorKey => $errorRow) {
                            ...
                            if (
                                $exceptionArray[$errorKey]['resolve_action'] == 'reassign' and in_array($exceptionArray[$errorKey]['field'], ['customer_account_number', 'objectives', 'rule_engine'])
    ...
    {
        $return = [
            'licence_id' => 0, 'broker_id' => $pBroker_id, 'first_name' => '', 'last_name' => '', 'active_check' => 0, 'state_id' => (int)$pClient_state, 'state_name' => '',
    ...
    function dazl_security_row($data = '')
    {
        ...
        if (!empty($data)) {
            ...
            if ($sequenceNumber == '01') {
                $return = [
                    "sequence_number" => $sequenceNumber, "management_code" => substr($data, 7, 3), "fund_number" => substr($data, 10, 5), "cusip_number" => substr($data, 15, 9), "load_code" => substr($data, 24, 1), "ticker_symbol" => substr($data, 25, 7), "fund_type" => substr($data, 32, 2), "objective_code" => substr($data, 36, 2)
                ];
            } else if ($sequenceNumber == '02') {
                $return = [
                    "sequence_number" => $sequenceNumber, "fund_name" => substr($data, 7, 50), "fund_name2" => substr($data, 57, 40)
                ];
            }
        }
        return $return;
    ...
    function dazl_account_row($data = '')
    {
        ...
        if (!empty($data)) {
            ...
            if ($sequenceNumber == '01') {
                $return = [
                    "sequence_number" => $sequenceNumber, "management_code" => substr($data, 7, 3), "cusip_number" => substr($data, 10, 9), "fund_number" => substr($data, 19, 5), "ticker_symbol" => substr($data, 24, 7), "fund_name" => substr($data, 31, 50), "fund_name2" => substr($data, 81, 394)
                ];
            } else if ($sequenceNumber == '02') {
                $return = [
                    "sequence_number" => $sequenceNumber, "broker_identification_number" => substr($data, 14, 25), "customer_account_number" => substr($data, 39, 20), "ssn_format_code" => substr($data, 62, 1), "social_security_number" => substr($data, 63, 9), "ssn_certification_code" => substr($data, 72, 1), "ssn_format_code2" => substr($data, 73, 1), "social_security_number2" => substr($data, 74, 9), "ssn_certification_code2" => substr($data, 83, 1), "customer_date_of_birth" => substr($data, 84, 8), "customer_date_of_birth2" => substr($data, 92, 8), "sex" => substr($data, 102, 1), "plan_type" => substr($data, 103, 1)
                ];
            } else if ($sequenceNumber == '03') {
                $return = [
                    "sequence_number" => $sequenceNumber, "registration_line1" => substr($data, 7, 40), "registration_line2" => substr($data, 47, 40), "internal_account_number" => substr($data, 87, 10), "marketing_source" => substr($data, 97, 19)
                ];
            } else if ($sequenceNumber == '04') {
                $return = [
                    "sequence_number" => $sequenceNumber, "registration_line3" => substr($data, 7, 40), "registration_line4" => substr($data, 47, 40)
                ];
            } else if ($sequenceNumber == '05') {
                $return = [
                    "sequence_number" => $sequenceNumber, "registration_line5" => substr($data, 7, 40), "registration_line6" => substr($data, 47, 40), "birth_date_529" => substr($data, 87, 8)
                ];
            } else if ($sequenceNumber == '06') {
                $return = [
                    "sequence_number" => $sequenceNumber, "first_address_line" => substr($data, 7, 1), "line_code" => substr($data, 7, 1), "open_date" => substr($data, 8, 8), "closed_date" => substr($data, 16, 8), "last_maintenance_date" => substr($data, 24, 8), "alpha_code" => substr($data, 32, 20), "roa_number" => substr($data, 54, 10), "letter_of_intent_indicator" => substr($data, 70, 1), "letter_of_intent_expiration" => substr($data, 71, 8), "employee_code" => substr($data, 116, 1)
                ];
            } else if ($sequenceNumber == '07') {
                $return = [
                    "sequence_number" => $sequenceNumber, "city" => substr($data, 7, 40), "state" => substr($data, 47, 3), "zip_code" => substr($data, 50, 9), "address_code" => substr($data, 59, 1), "office_phone" => substr($data, 60, 10), "home_phone" => substr($data, 70, 10), "nav_account" => substr($data, 82, 1), "dealer_number" => substr($data, 83, 6), "branch_number" => substr($data, 89, 9), "representative_number" => substr($data, 98, 9), "loi_start_date" => substr($data, 107, 8)
                ];
            } else if ($sequenceNumber == '08') {
                $return = [
                    "sequence_number" => $sequenceNumber, "representative_sortname" => substr($data, 7, 25), "representative_name" => substr($data, 32, 40), "rpo_indicator" => substr($data, 72, 1), "rpo_date" => substr($data, 73, 8), "marketing_option" => substr($data, 81, 1), "email" => substr($data, 82, 36)
                ];
            }
        }
        return $return;
    ...
    function dazl_comm_row($data = '')
    {
        ...
        if (!empty($data)) {
            ...
            if ($sequenceNumber == '01') {
                $return = [
                    "sequence_number" => $sequenceNumber, "management_code" => substr($data, 7, 3), "dealer_number" => substr($data, 10, 6), "dealer_branch_number" => substr($data, 16, 9), "representative_number" => substr($data, 50, 9), "representative_name" => substr($data, 59, 25), "fund_number" => substr($data, 84, 5), "cusip_number" => substr($data, 89, 9), "customer_account_number" => substr($data, 98, 20), "customer_account_state" => substr($data, 118, 2), "social_code" => substr($data, 120, 2), "social_security_number" => substr($data, 122, 9), "customer_name" => substr($data, 131, 20), "process_date" => substr($data, 151, 8), "trade_date" => substr($data, 159, 8), "transaction_code" => substr($data, 167, 1), "dealer_commission_amount" => substr_replace(substr($data, 168, 9), '.', 7, 0), "dealer_commission_sign_code" => substr($data, 177, 1), "commission_rate" => substr($data, 178, 4), "commission_type" => substr($data, 186, 1), "share_quantity" => substr_replace(substr($data, 188, 13), '.', 10, 0), "share_price" => substr_replace(substr($data, 201, 5), '.', 3, 0), "letter_of_intent_number" => substr($data, 206, 10), "direct_indirect_indicator" => substr($data, 216, 1), "alpha_code" => substr($data, 217, 10)
                ];
            }
        }
        return $return;
