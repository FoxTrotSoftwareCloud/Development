	...
	class import extends db{
        ...
        public function resolve_exceptions($data){
            ...
            if(isset($exception_file_type) && $exception_file_type == '1'){
                // Broker Not Found
                if ($error_code_id == 1) {
                    // Only option is to reassign broker to an existing one
                    $result = $this->resolve_action_3reassign('broker_id', $rep_for_broker, $exception_file_type, $exception_file_id, $exception_data_id, $exception_record_id);
                }
                // Terminated Broker
                else if($error_code_id == 3){
                    ...
                    if($this->errors!=''){
                        ...
                        // CLIENT FILE - Terminated - 2-> RESET u5 Date
                            ...
                            else if($resolveAction == 3)
                            {
                                ...
                                if($res){
                                    $q1 = "UPDATE `".IMPORT_EXCEPTION."`"
                                            ." SET `solved` = 1"
                                                .",`is_delete` = 1"
                                                .",`resolve_action` = 4"
                                                .$this->update_common_sql()
                                            ." WHERE `file_id`='".$exception_file_id."'"
                                            ." AND `temp_data_id`='".$exception_data_id."'"
                                    ;
            ...
            //--- IDC EXCEPTIONS ---//
            if(isset($exception_file_type) && $exception_file_type == '2') {
                ...
                else if($exception_field == 'representative_number')
                {
                    switch ($error_code_id){
                        case 13:
                            ...
                            $q = "SELECT id,first_name,last_name FROM `".BROKER_MASTER."` WHERE `is_delete`=0 AND `id`=".$data['rep_for_broker'];
                            $res = $this->re_db_query($q);

                            if($this->re_db_num_rows($res) == 0)
                            {
                                $this->errors = "Broker #".$data['rep_for_broker']." not found.";
                            } else {
                                $row = $this->re_db_fetch_array($res);

                                // Resolve Exception - code 30 Assign user-defined broker\rep to trade
                                $q = "UPDATE `".IMPORT_IDC_DETAIL_DATA."`"
                                    ." SET `broker_id` =".$data['rep_for_broker']
                                            .$this->update_common_sql()
                                    ." WHERE `id`=".$exception_data_id
                                    ." AND `file_id`=".$exception_file_id
                                ;
                                ...
                                if($res)
                                {
                                    ...
                                    $q = "UPDATE `".IMPORT_EXCEPTION."`"
                                            ." SET `resolve_action`=3"
                                                .",`resolve_assign_to`='".$data['rep_for_broker']."'"
                                                    .$this->update_common_sql()
                                            ." WHERE `id`=".$exception_record_id
                                    ;
                                    $res = $this->re_db_query($q);
            ...
            if (in_array($file_type,[1,2,3]) AND $file_id>0 AND $new_client_id>0 AND $detail_id>0 AND $exception_record_id>0){
                ...
                if($this->re_db_num_rows($res))
                {
                    ...
                    if($return)
                    {
                        ...
                        $result = $this->reprocess_current_files($file_id, $file_type, $detail_id);
                    }
                }
            }
        ...
        // ASSIGN import record TO ANOTHER /[client_id][broker_id]/
        function resolve_action_3reassign($reassign_field='', $newId=0, $file_type=0, $file_id=0, $detail_data_id=0,  $exception_record_id=0){
            $result = 0;
                        ...
                        ********************/
                        if ($result == 0) {
                            ...
                            if ($existingSocialArray OR $existingAccountArray){
                                if ($existingSocialArray){
                                    $clientId = $existingSocialArray[0]['id'];
                    ...
                    foreach($check_idc_array as $check_data_key=>$check_data_val){
                        ...
                        if(empty($check_data_val['cusip_number'])){
                            ...
                            if($this->re_db_num_rows($resCusipFind)>0){
                                ...
                                $product_id = $foundCusip['id'];
                                $product_category_id = $foundCusip['category'];
                            } else {
                                $q = "INSERT INTO `".IMPORT_EXCEPTION."`"
                        ...
                        if(!$reassignClient AND empty($check_data_val['customer_account_number'])){
                            ...
                            // Objectives Check
                            if (!empty($client_id) AND !empty($foundCusip['objective'])){
                            ...
                            if (!empty($batch_id)){
                                ...
                                $q1 = "INSERT INTO `".TRANSACTION_MASTER."`"
                                        ." SET `file_id`='".$check_data_val['file_id']."'"
                                            ...
                                            .",`trail_trade`='".($check_data_val['commission_record_type_code']=='3' ? '1' : '0')."'"
                                            .$con
                                            .$this->insert_common_sql();
                                $res1 = $this->re_db_query($q1);
                                $last_inserted_id = $this->re_db_insert_id();
