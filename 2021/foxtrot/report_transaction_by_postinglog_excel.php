    ...
    foreach ($return as $trans_main_key => $trans_main) {
        ...
        foreach ($trans_main["child"] as $trans_main_key => $trans_main_data) {
            ...
            foreach ($trans_main_data as $key => $val) {
                ...
                if ($val['trade_date'] != '0000-00-00') {
                    $trade_date = date('m/d/Y', strtotime($val['trade_date']));
                }
                if ($val['commission_received_date'] != '0000-00-00') {
                    $commission_received_date = date('m/d/Y', strtotime($val['commission_received_date']));
                }
        ...
        $i++;
        $sheet_data[0]['D' . $i] = array('*** BATCH TOTALS ***', array('bold', 'right', 'color' => array('000000'), 'background' => array('f1f1f1'), 'size' => array(10), 'font_name' => array('Calibri'), 'merge' => array('D' . $i, 'F' . $i)));
        $sheet_data[0]['G' . $i] = array($instance->re_db_output('$' . number_format($total_batch_amount_invested, 2)), array('bold', 'right', 'color' => array('000000'), 'background' => array('f1f1f1'), 'size' => array(10), 'font_name' => array('Calibri')));
        $sheet_data[0]['H' . $i] = array($instance->re_db_output('$' . number_format($total_batch_charges, 2)), array('bold', 'right', 'color' => array('000000'), 'background' => array('f1f1f1'), 'size' => array(10), 'font_name' => array('Calibri')));
        $sheet_data[0]['I' . $i] = array($instance->re_db_output('$' . number_format($total_batch_commission_received, 2)), array('bold', 'right', 'color' => array('000000'), 'background' => array('f1f1f1'), 'size' => array(10), 'font_name' => array('Calibri')));
    }
    $i++;
