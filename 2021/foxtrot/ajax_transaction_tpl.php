...
if(
   ...
   if(isset($_GET['action']) && $_GET['action'] == 'split_commission'){
      ...
      foreach($edit_split as $editSplitKey=>$editSplitVal){  
         $doc_id2++; 
         $splitBroker = ($broker_id>0 ? $editSplitKey['rap'] : $editSplitKey['split_broker']);
         $splitRate = ($broker_id>0 ? $editSplitKey['rate'] : $editSplitKey['split_rate']);
      ?>
         <tr class="tr" data-rowid="<?php  echo $editSplitVal['broker_id'] ?>">
            <!-- Column 1: Split Receiving Rep -->
            <td>
               <input type="hidden" name="split[]" value="<?php echo json_encode(['transaction_id'=>$transaction_id, 'broker_id'=>$broker_id]) ?>" />
               <select name="split_rep[]"  class="form-control">
                  <option value="">Select Broker</option>
                  <?php foreach($select_broker as $key => $val) {
                     if($val['id'] != $broker_id){?>
                        <option <?php if(isset($editSplitVal['rap']) && $editSplitVal['rap']==$val['id']) {?>selected<?php } ?> value="<?php echo $val['id']?>"><?php echo $val['first_name'].' '.$val['last_name']?></option>
                     <?php } } ?>
               </select>
            </td>
            <!-- 2: Split Rate -->
            <td>
               <div class="input-group">
                  <input type="number" name="split_rate[]" step="0.01" onchange="handleChange(this);" value="<?php echo $editSplitVal['rate'];?>" class="form-control" /><span class="input-group-addon">%</span>
               </div>
            </td>
      ...
      <tr id="add_rate">
         ...
         <td>
            <div class="input-group">
               <input type="number" name="split_rate[]" onchange="handleChange(this);" step="0.01" value="" class="form-control" /><span class="input-group-addon">%</span>
            </div>
         </td>
