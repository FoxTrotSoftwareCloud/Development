...
$ria_category_id = 12;
$get_ria_products = $instance->get_product($ria_category_id);
...
if (
    ...
    $postData['product_cate'] = $ria_category_id;
    $commission_received_int = $postData['split-commission-recived'];

    // below loop use for insert data in transaction table
    for($i = 0;$i<$postData['payroll_count'];$i++){
        $postData['trade_date'] =$postData['split-trade-date'][$i];
        $postData['commission_received'] = $commission_received_int[$i];
    ...
    header('location:report_transaction_by_batch.php?batch_id=' . $batch_view_id);
    exit;
